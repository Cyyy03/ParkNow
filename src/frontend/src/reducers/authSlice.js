...
import { updateAgentsPresence } from '../reducers/inboxAgentsSlice';
    ...
    try {
      ...
      if (response && response.status === 401) {
        ...
        if (hasAuthErrorMsg) {
          ...
          showToast({ message: "failed authentication" });
        }
        if (!errors) {
          throw errors;
        }
      ...
      try {
        ...
        if (response && response.status === 401) {
          ...
          if (hasAuthErrorMsg) {
            ...
            showToast({ message: "failed authentication", type: 'error' });
          }
        } else {
...
export const selectAccounts = state => state.auth.userData.accounts;

export const selectUser = state => state.auth.userData;

export const selectUserId = state => state.auth.userData.id;

export const selectLoggedIn = state => state.auth.userData?.id;
